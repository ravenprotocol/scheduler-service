...
def vertical_split(graph_id, minimum_split_size):
    ...
    for subgraph_id in op_dependency:
        ...
        subgraph_ops = ravdb.get_subgraph_ops(
            graph_id=graph_id, subgraph_id=subgraph_id)
        subgraph_op_ids = []
            ...
            if subgraph.status != 'failed':
                if subgraph.status == 'standby':
                    ...
                    if parent_subgraph.status == SubgraphStatus.COMPUTED:
                        ravdb.update_subgraph(subgraph, op_ids=str(
                            subgraph_op_ids), status='not_ready', complexity=1)
                    else:
                        ...
                        for standby_op_id in standby_ops_ids:
                            ...
                            if standby_op.inputs != 'null':
                                standby_op_inputs = ast.literal_eval(
                                    standby_op.inputs)
                                for standby_op_input_id in standby_op_inputs:
                                    standby_op_input = ravdb.get_op(
                                        standby_op_input_id)
                        ...
                        if not standby_flag:  # or parent_subgraph.status == SubgraphStatus.COMPUTED:
                            # if parent_subgraph.status == SubgraphStatus.COMPUTED:
                            ravdb.update_subgraph(
                                subgraph, op_ids=str(subgraph_op_ids), status='not_ready',
                                complexity=1,
                            )
                        ...
                        for input_id in ast.literal_eval(op.inputs):
                            ...
                            if input_id in computed_ops:
                                ...
                                ghost_op = ravdb.create_op(
                                    name=name, graph_id=input_op.graph_id,
            ...
            if len(op_ids) > minimum_split_size:
                ...
                if parent_subgraph is not None:
                    if str(row1) == str(parent_subgraph.op_ids):
                        minimum_split_size += random.randint(
                            1, len(op_ids) - minimum_split_size)
                        row1 = op_ids[:minimum_split_size]
                ...
                if len(row2) > 0:
                    ...
                    for op_id in row2:
                        op = ravdb.get_op(op_id)
                        ravdb.update_op(
                            op, subgraph_id=new_subgraph.subgraph_id)
    ...
    while True:
        ...
        if len(distributed_graphs) == 0 and len(federated_graphs) == 0:
            ...
            for federated_graph in federated_graphs:
                created_subgraphs = ravdb.get_subgraphs_from_graph(
                    federated_graph.id)
            ...
            for distributed_graph in distributed_graphs:
                ...
                ravdb.update_graph(distributed_graph, inactivity=int(
                    distributed_graph.inactivity) + 1)
                ...
                if if_failed_subgraph is not None:
                    ...
                    ravdb.update_graph(distributed_graph,
                ...
                for subgraph_id in failed_subgraph_ids:
                    subgraph = ravdb.get_subgraph(
                        subgraph_id=subgraph_id, graph_id=current_graph_id)
                ...
                subgraphs = ravdb.get_ready_subgraphs_from_graph(
                    graph_id=current_graph_id)
                ...
                for subgraph in subgraphs:
                    ...
                    if idle_clients is not None:
                        ...
                        if bool(prelim_times):
                            previously_assigned_client = ravdb.get_assigned_client(
                                subgraph_id=subgraph.subgraph_id,
                                graph_id=subgraph.graph_id,
                            )
                            if previously_assigned_client is not None:
                                ravdb.update_subgraph(
                                    subgraph, status='assigned', complexity=150)
                            else:
                                fastest_client_id = min(
                                    prelim_times, key=prelim_times.get)
